...
def load_dataset(dataset, root , tables=["diagnoses_icd", "procedures_icd", "prescriptions"], task_fn = None, dev = False):
    if dataset=='mimic3':
        ...
        return load(root)
    # dataset.set_task(task_fn=task_fn)
...
# def mm_dataloader(trainset, validset, testset, batch_size=64, sampler):
#     train_loader = DataLoader(trainset, batch_size=batch_size, shuffle=True, collate_fn=custom_collate_fn, sampler=sampler)
#     val_loader = DataLoader(validset, batch_size=batch_size, shuffle=True, collate_fn=custom_collate_fn, sampler=sampler)
#     test_loader = DataLoader(testset, batch_size=batch_size, shuffle=True, collate_fn=custom_collate_fn, sampler=sampler)
